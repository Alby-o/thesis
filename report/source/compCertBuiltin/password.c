...
struct user_t {
    user_t* next;
    char name[BUFF_LEN];
    char password[BUFF_LEN];          //           { L(password) = High }
    size_t balance;
};
...
user_t* setup_users() {
    ...
    __builtin_annot("L(%1) = high", user_admin->password);
    ...
    __builtin_annot("L(%1) = high", user_alice->password);
    ...
    __builtin_annot("L(%1) = high", user_abdul->password);
