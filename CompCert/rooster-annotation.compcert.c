...
{
  int sum;
  __builtin_annot("# file:rooster-annotation.c line:7 function:fun\n%here CRITICAL COMMENT");
  sum = a + b + c;
  if (sum < 0) {
    return sum;
  }
...
{
  int result;
  __builtin_annot("# file:rooster-annotation.c line:27 function:main\n%here L(%e1) = high",
                  goose);
  __builtin_annot("# file:rooster-annotation.c line:28 function:main\n%here EXCEPTIONAL");
  rooster = 1;
  drake = 5;
  goose = 10;
